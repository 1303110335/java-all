 ...
 * 数据集1 ： 1 3 5 7 9
 * 数据集2 ： 2 4 6 8 10
 * 取出最大或最小的5个数：1, 2, 3, 4, 5
 *
 * 1.首先利用取出每个数据集中的第一个数，并放入优先队列中进行排序，如果是正序，则最小的数据集放在队列的顶部
 * 2.取出该数据集，将最小的数据放入到结果集中，然后判断该数据集是否还有数据，将下一个数据放入到orderValues并将数据集重新放入到优先队列中，
 * 3.重复第二步，直到达到limit的条件
 * @author xuleyan
